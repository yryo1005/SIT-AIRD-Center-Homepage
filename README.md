...
研究内容ページ（`src/pages/basic-research/index.html`。表示上の見出しは「研究内容」）に並ぶ研究テーマも，ニュースと同じ「1件＝1つのJSONファイル」の仕組みです。組込AI・画像処理・生成AI・音声AI・医療AI・NLP・基礎理論など，あらゆる分野の研究テーマをこの1箇所に集約しています（対応する研究分野のタグ一覧は，研究内容ページの`.field-tags`にHTML直書きで表示しており，新しい分野を追加した場合はここも合わせて更新してください）。
...
## ダウンロード資料（紹介ポスター等）を追加・更新する方法

トップページ・在学生の方へページ・高校生の方へページには，紹介ポスター（PDF）をダウンロードできるボタンがあります（order_017対応で追加）。

1. `src/assets/documents/`の下に，配布したいファイル（PDF等）を置きます。現在は`ai-rd-center-poster.pdf`（紹介ポスター）が置かれています。
2. HTML側では，`<a data-download="ai-rd-center-poster.pdf" href="#" ...>`のように，`data-download`属性にファイル名を指定します（`href`はダミーで構いません）。ページ読み込み時に`src/shared/script.js`の`wireDownloadLinks()`が，実際にダウンロード可能なURLへ自動的に書き換えます。
3. **同じファイル名のPDFを差し替えたい場合**は，`src/assets/documents/`内の該当ファイルをそのまま上書きするだけで反映されます。新しい資料を追加したい場合は，`src/assets/documents/`に新しいファイルを置き，追加したいページに同様の`data-download`属性を持つリンクを配置してください。
4. ファイルを保存し、`git add`・`git commit`・`git push`すると，数分後にGitHub Pages上に反映されます。

（画像と同様，PDFのファイルパスをJavaScript経由で解決しているのは，`<a href="...">`のような属性はViteの標準アセット解析の対象に含まれないためです。詳しくは「フォルダ構成」内の`src/shared/script.js`の説明を参照。）

## 業績一覧を追加・編集する方法

トップページ・研究内容ページの両方に表示している「業績一覧」は，`src/data/publications.js`を唯一のデータソースとして共有しています（order_018対応。以前は両ページに同じ内容が別々にハードコードされていました）。

1. `src/data/publications.js`の`PUBLICATION_CATEGORIES`配列を編集します。各分類は`{ key, label, items }`の形で，`items`は業績1件ごとの文字列（著者・タイトル・掲載誌等）の配列です。
2. 分類名の末尾に付く「（◯件）」という件数表示は，`items`の配列の長さから自動的に計算されるため，手動で書き換える必要はありません。
3. 両ページのHTML側は`<div class="accordion-group" data-source="publications-json"></div>`という空のマウントポイントになっており，`src/shared/script.js`の`renderPublicationsAccordion()`が上記データから内容を描画します（初期状態は閉じたアコーディオンです）。
...
│   │   │   └── join/         ← 在学生の方へページの活動写真（対外的な活動・メンバー同士の交流）
│   │   └── documents/        ← ダウンロード資料（紹介ポスターPDF等。後述）
│   └── pages/
│       ├── top/index.html
│       ├── news/index.html
│       ├── facility/index.html
...
- 各ページ上部（ページヒーロー直下）には，そのページ内の見出し（h2）へジャンプする目次ナビゲーション（`<nav class="toc-nav" data-toc>`）を配置しています（order_018）。手作業で目次のリンク文言を書くのではなく，`initTableOfContents()`が実際のDOM構造（各セクションの見出しテキスト）から動的に生成するため，見出しの追加・変更・削除をしても目次側の修正は不要です。見出しがないページ（ニュース・施設・学会行脚マップ等，単一セクションのみのページ）では目次欄は自動的に非表示になります。
- 学会行脚マップページのサイドパネルには，登録済みの全都道府県の写真をランダムな順序で数秒おきに切り替え表示するミニカルーセルを配置しています（order_018）。表示の仕組み自体は，ニュース写真帯・施設の部屋写真と同じ共通ミニカルーセル部品（`buildMiniCarouselHtml()`/`wireMiniCarousel()`）です。
...
- 学生の声のデータファイル化（`src/data/students/`）：井上來彌・渡邉光喜の2名分のみ実施済み（order_016）。`src/pages/basic-research/index.html`に残る4件（渡部朔冶・石川悠樹・濵田聖・山富龍）は未移行のままHTML直接記述となっている。
