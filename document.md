...
## 28. メッセージ性の強化，年間スケジュール・研究分野拡充・スキル/進路・設備機材の追加，トップページへの対象者別ボタン配置（order_019）の内容

`.orders/order_019.md`は，5項目にわたる追加・修正仕様（メッセージ性，年間スケジュール，研究内容拡充，進路・スキル，機材・加入方法）と，トップページのボタン配置追加指示である。

- **メッセージ性の強化**：トップページのヒーロー文，joinページの「AI R&D Centerとは」，for-highschoolページのヒーロー文に，「初心者からでも学会発表を目指せる」「やりたいテーマを自由に追求できる」「本気で情熱を持って取り組む学生ほど大きく成長・飛躍できる」という3つの軸を明記した。既存の実績（学部2年での学会発表挑戦例等）と矛盾しない範囲の表現とした。
- **年間スケジュールの新設**：joinページに「年間スケジュール」セクションを新設し，2年前期〜4年後期の6期分の活動内容を縦型タイムライン（`.year-timeline`，CSS新設）で掲載した。あわせて，3月・9月の「登竜門となる学会」（電子情報通信学会総合大会／情報処理学会全国大会，電子情報通信学会ソサイエティ大会／情報処理学会FIT）と，さらに上位の発表機会（研究会発表・国際学会・学術論文誌）を掲載した。内容はユーザー提供のテキストをそのまま構造化して転記し，新たな推測は加えていない。
- **研究内容の拡充**：研究分野の紹介文（トップページ・研究内容ページの複数箇所）に「生成AI」「音声AI」「医療AI」を追加し，研究内容ページには研究分野タグ一覧（`.field-tags`）を新設した。新規研究テーマとして「超音波画像における血管領域自動検出AI（医療AI）」を`src/data/research/17-vessel-detection-ai.json`に追加した（背景・目的，技術アプローチはユーザー提供テキストのまま反映。この研究テーマは既存の業績一覧`src/data/publications.js`内の該当2件と対応しており，事実として確認できる）。
- **身につくスキル・卒業後の進路の新設**：joinページ・for-highschoolページの両方に「身につくスキル」（実践的なプログラミング能力，最新のAI・機械学習・深層学習知識，プレゼンテーション・論文執筆スキル）と「卒業後の進路・キャリア」（IT企業就職に加え，大学院進学率が高く毎年多くの進学者を輩出）のセクションを新設した。進学率の具体的な数値は確認できないため記載していない。
- **設備・機材，加入方法の更新**：joinページに「設備・機材」セクションを新設し，指定のGPU搭載PC構成（RTX PRO 6000／RTX 4090／RTX 4060 Ti Super／RTX 3090各2台，GPU搭載ノートPC複数台）を掲載した。「参加方法」セクションを更新し，1年生は年度末（1月末）の「情報学実習」授業内ガイダンス，2〜3年生・年度途中の希望者は二宮先生への直接相談，という2パターンの加入・見学方法を明記した。
- **トップページへの対象者別ボタン配置**：トップページのヒーロー内「紹介ポスターをダウンロード（PDF）」ボタンの右側に，「在学生の方へ」「高校生の方へ」への2つのリンクボタンを右寄せ（`margin-left: auto`）で追加配置した。
